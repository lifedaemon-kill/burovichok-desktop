...
// AddOilFieldEntry добавляет одну запись в oilfield
func (p *Postgres) AddOilFieldEntry(ctx context.Context, name string) error {
	sqlStr := `INSERT INTO oilfield (name) VALUES ($1) ON CONFLICT (name) DO NOTHING`
	_, err := p.DB.ExecContext(ctx, sqlStr, name)
	if err != nil {
		return errors.Wrap(err, "executing AddOilFieldEntry query")
	}
	return nil
}

// AddProductiveHorizonEntry добавляет одну запись в productive_horizon
func (p *Postgres) AddProductiveHorizonEntry(ctx context.Context, name string) error {
	sqlStr := `INSERT INTO productive_horizon (name) VALUES ($1) ON CONFLICT (name) DO NOTHING`
	_, err := p.DB.ExecContext(ctx, sqlStr, name)
	if err != nil {
		return errors.Wrap(err, "executing AddProductiveHorizonEntry query")
	}
	return nil
}

// AddInstrumentTypeEntry добавляет одну запись в instrument_type
func (p *Postgres) AddInstrumentTypeEntry(ctx context.Context, name string) error {
	sqlStr := `INSERT INTO instrument_type (name) VALUES ($1) ON CONFLICT (name) DO NOTHING`
	_, err := p.DB.ExecContext(ctx, sqlStr, name)
	if err != nil {
		return errors.Wrap(err, "executing AddInstrumentTypeEntry query")
	}
	return nil
}

// AddResearchTypeEntry добавляет одну запись в research_type
func (p *Postgres) AddResearchTypeEntry(ctx context.Context, name string) error {
	sqlStr := `INSERT INTO research_type (name) VALUES ($1) ON CONFLICT (name) DO NOTHING`
	_, err := p.DB.ExecContext(ctx, sqlStr, name)
	if err != nil {
		return errors.Wrap(err, "executing AddResearchTypeEntry query")
	}
	return nil
}
